...
<header class="sticky top-0 z-50 border-b border-black/10 bg-white/95 backdrop-blur">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 lg:px-8">
        ...
        <nav class="hidden gap-5 text-sm font-semibold xl:flex"><a href="{{ route('club') }}">Le Club</a><a href="{{ route('team') }}">Équipe</a><a href="{{ route('matches') }}">Matchs</a><a href="{{ route('news.index') }}">Actualités</a><a href="{{ route('gallery') }}">Galerie</a><a href="{{ route('partners') }}">Partenaires</a><a href="{{ route('contact') }}">Contact</a></nav>
        <a href="{{ route('recruitment') }}" class="rounded-full bg-orange-500 px-4 py-2 text-sm font-bold text-white">Rejoindre le club</a>
    </div>
</header>
